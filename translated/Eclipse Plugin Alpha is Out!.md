...
IntelliJ IDEA中的Kotlin支持一直是我们的首要任务之一，但我们也一直致力于Eclipse支持，今天我们很高兴地介绍这一努力的第一个alpha结果。
...
要试一下，您将需要一个干净的Eclipse Luna安装。 Kotlin插件可从Eclipse Marketplace获得。安装Kotlin插件的最简单方法是将该按钮拖放到正在运行的Eclipse窗口中：
...
或者，您可以使用帮助 - > Eclipse Marketplace ...菜单或以下更新站点：
...
在编辑Kotlin来源时，许多快速修复和意图将是有用的，我们将逐渐添加越来越多的这些。我们从简单的方法开始，用索引运算符（[...]）替换get（）调用：
...
Open Declaration是本版本中唯一支持的导航操作：
...
此外，您可以使用Outline View导航到Kotlin文件中的声明，并在Open Resource窗口中按名称搜索Kotlin文件。
### 调试器
...
如果您在类路径中已经有JUnit 3或JUnit 4，则可以在Kotlin中编写测试。运行 - >运行方式 - > Kotlin JUnit测试在主菜单中开始测试并在JUnit视图中打开结果。
...
我们可以慢慢地继续实现功能，使插件越来越稳定和强大。反而我们决定显示早期的阿尔法希望反馈。对我们来说非常重要。
我们的跟踪器中已经有一些问题和功能请求。请填写免费添加更多。谢谢。
此外，这是一个开放源码的项目，我们期望它比我们的编译器和IntelliJ Plugin更加是一个社区的努力，所以您的Pull请求非常受欢迎。
