...
我们很高兴为Eclipse IDE提供一个新版本的插件。随着对Kotlin 1.0.2编译器的支持，此更新带来了非常重要的功能和改进。

{% raw %}
<p><span id="more-3901"></span></p>
{% endraw %}

该版本中重新编写了代码格式化功能。而不是我们的第一个天真的实现，我们主要设法将高级格式化程序从Kotlin IntelliJ Idea插件移植到Eclipse中。这意味着很多修复已经在那里，即将到来的改进将自动获取！
...
可以添加缺少的类，从0.1.0版本逐个导入一个快速修复，现在我们通过引入组织导入功能进行了改进。它将清理未使用的导入，为文件中使用的类添加缺少的导入，并使用它们。
