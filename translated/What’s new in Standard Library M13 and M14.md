...
懒惰评估是一种有用的模式，将其仅限于委托的属性是不公平的。现在在kotlin包中有Lazy <T>类型，您可以在许多上下文中使用它。您可以将属性委派给Lazy实例，该实例具有get extension函数以满足委派约定：
...
与Delegates.lazy不同，默认情况下，延迟语义会同步。
## 将属性委派给地图

另一个常见的用例是将属性的值存储在地图中，其中属性的名称用作键。对于这种情况，我们提供在地图上获取和设置的扩展函数以满足委派约定，允许将属性委托给地图，而不创建包装器委托对象：
...
在M13中，我们正在确定Char类型的算术运算的语义变化。只剩下三个算术运算：

* Char  -  Int→Char
* Char + Int→Char
* Char  -  Char→Int

涉及Char的所有其他二进制操作在M12中已被弃用，现在已被删除。
## 加和减运算符集合

加上对集合的操作不是新的，但是它被定义为使得它的返回类型和行为不直观：例如，当将一个元素添加到集合中时，您将收到一个列表，该列表将该元素连接到集合的所有元素，可能导致该元素被复制。
现在返回的加号类型取决于第一个操作数的类型：
...
我们也引入了减运算。它的第一个操作数的类型与其返回类型之间具有相同的关系以及以下语义：
...
在M13之前，标准库中有两个所谓的范围函数：let和with。我们称之为范围函数，因为它们的唯一目的是修改作为最后一个参数传递的函数的作用域。例如，将范围函数的参数带入函数参数的接收方，以及在其他情况下反之亦然。
现在，您还可以使用另外两个示波器功能：
...
有一个共同的要求，在科特林推出正确的范围。我们检查了用例，发现它们大多数都涉及整数范围。在整数的情况下，在结束范围内打开可以用闭合范围表示，其结束值比相应的开放范围的结束值小一个。
要创建这样的范围，您可以使用新引入的功能。它返回闭合范围，其值直到指定的结尾，但不包括它，因此：
0至20 == 0..19
## 找到vs firstOrNull

有一些关于命名一个函数的争论，这个函数找到了一个集合中匹配给定谓词的第一个元素。 find很容易探索，但firstOrNull与LINQ和Reactive Extensions中如何调用此操作是一致的。在M13之前，发现已经被弃用为支持firstOrNull，但是现在我们已经决定“不弃用”它，并且作为firstOrNull的同义词。我们也提供了findLast，它是lastOrNull的同义词。
## 从多行字符串文字中删除缩进
...
在M13中，我们删除了以前不推荐使用的一些API：流，迭代器的扩展方法，FunctionalList，FunctionalQueue和StringTemplate。
完整的更改列表可在此处获得。
# M14计划

## 在包装零件之间分配顶层功能

正如我们之前所宣布的，我们正在改变顶层功能和属性如何映射到编译代码中的类文件的方式。在下一个里程碑中，标准库中的所有顶级内容将分布在相应的包装部件之间。我们将保留包含所有顶级功能的KotlinPackage facade类，但将来它将被废弃和删除。
