...
通常情况如何：您有两个模块，`a`和`b`，而在`a`中，您有一个顶级的函数在<code > foo.bar`包。一切都很好，直到您在另一个模块</strong>，`b`中的相同`foo.bar`软件包<strong>中添加另一个顶级功能。一旦你这样做，两个模块都会生成具有相同完全限定名称的类文件：`foo.bar.BarPackage`，并且运行时没有机会区分它们。并且您得到一个`NoSuchMethodError`，因为在运行时只加载了两个外观中的一个，而另一个外部的函数不在那里。
（编译也可能会打破，但并不是那么糟糕。）
## 新设计
