...
很像IntelliJ IDEA，要安装Kotlin插件，您需要单击首选项（设置）并选择插件条目。最简单的方法是在对话框弹出时立即开始输入*插件*。
...
虽然我们可以从磁盘下载插件并进行安装，但最简单的方法是单击安装JetBrains插件...*，然后从列表中选择Kotlin。右键单击并选择*下载并安装*
...
其中项目的源代码位于文件夹*main / java*之下。由于我们想使用Kotlin（我们可以将Java和Kotlin混合使用在同一个项目中），因此我们需要在*main*下创建一个名为“kotlin”的新文件夹。*在Gradle脚本中，我们稍后将此文件夹定义为源代码。
...
我们需要在Gradle配置中设置一些依赖项和源文件夹。打开*build.gradle*文件并复制以下内容
...
指定*sourceSet*将自动将我们之前创建的文件夹（*main / kotlin*）标记为Android Studio中的源根。
